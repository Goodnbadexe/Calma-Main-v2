...
1. Keep overlay but ensure it never modifies `overflow` or layout; already done; remove it entirely if any residual jitter remains.

### Horizontal Stats on Home

1. Force `.panorama-stats` to a single row on desktop: 6 columns with consistent gaps.

### Reorder Home Sections

1. Move “Stories From Our Community” above “Discover More” in `HomeLayout`.
